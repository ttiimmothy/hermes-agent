...
# Import ACHIEVEMENTS catalog from standalone file so the dashboard plugin
# system and other consumers can reference it without pulling in the full
# plugin API backend.  We insert the dashboard directory on sys.path to
# support loading via importlib.spec_from_file_location (the dashboard's
# plugin loader) where the containing package is not a proper Python
# package and relative imports would fail.
_ACH_DIR = str(Path(__file__).resolve().parent)
if _ACH_DIR not in _sys.path:
    _sys.path.insert(0, _ACH_DIR)

from achievements import ACHIEVEMENTS, tiers, req  # type: ignore[import-untyped]
