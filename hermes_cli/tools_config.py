...
CONFIGURABLE_TOOLSETS = [
    ...
    (
        "video_gen",
        "🎬 Video Generation",
        "video_generate (text/image/reference)",
    ),
    (
        "x_search",
        "🐦 X (Twitter) Search",
    ...
    ("sound_gen", "🎨 Sound Generation", "sound_generation"),
]
...
def _prompt_toolset_checklist(
    ...
    *,
    ...
    if disabled_toolsets:
        effective = [entry for entry in effective if entry[0] not in disabled_toolsets]
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    while True:
        ...
        if idx == _global_idx:
            ...
            if new_enabled != all_current:
                ...
                for ci, pk in enumerate(platform_keys):
                    ...
                    _disabled_ts_global = {
                        str(ts)
                        for ts in (_agent_cfg_global.get("disabled_toolsets") or [])
                    }
                    all_ts_global = _get_effective_configurable_toolsets()
                    total = len([
                        e for e in all_ts_global if e[0] not in _disabled_ts_global
                    ])
                    platform_choices[ci] = (
        ...
        _disabled_ts_p = {
            str(ts) for ts in (_agent_cfg_p.get("disabled_toolsets") or [])
        }
